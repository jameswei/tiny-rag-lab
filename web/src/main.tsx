...
export function App() {
  ...
  const [environmentKeyConfigured, setEnvironmentKeyConfigured] = useState(false);
  ...
  useEffect(() => { void api<{ configured: boolean; base_url?: string; model?: string; api_key_configured?: boolean }>("/provider-status").then((status) => { setEnvironmentProviderReady(status.configured); setEnvironmentKeyConfigured(Boolean(status.api_key_configured)); if (!localStorage.getItem("tiny-rag-lab-provider-url") && status.base_url) setProviderUrl(status.base_url); if (!localStorage.getItem("tiny-rag-lab-provider-model") && status.model) setProviderModel(status.model); }).catch(() => setEnvironmentProviderReady(false)); }, []);
