...
import type { CatalogQuestion, Evidence, IndexItem, LabRun, Stage } from "../types";
...
export function ExploreView({
  indexes, indexId, question, catalogQuestions, catalogQuestionId, retriever, topK, contextBudget, run, activeStage, lang, running, testingProvider, providerReady, onIndex, onQuestion, onCatalogQuestion, onRetriever, onTopK, onContextBudget, onStage, onRun, t,
}: {
  indexes: IndexItem[]; indexId: string; question: string; catalogQuestions: CatalogQuestion[]; catalogQuestionId: string; retriever: "dense" | "bm25" | "hybrid"; topK: number; contextBudget: number; run: LabRun | null; activeStage: Stage; lang: "en" | "zh"; running: "retrieve" | "ask" | null; testingProvider: boolean; providerReady: boolean;
  onIndex: (value: string) => void; onQuestion: (value: string) => void; onCatalogQuestion: (value: string) => void; onRetriever: (value: "dense" | "bm25" | "hybrid") => void; onTopK: (value: number) => void; onContextBudget: (value: number) => void; onStage: (value: Stage) => void; onRun: (kind: "retrieve" | "ask") => void; t: Copy;
}) {
  return <section className="view explore-view" aria-labelledby="explore-title">
    ...
    {catalogQuestions.length > 0 && <label className="question-field"><span>{t.catalogQuestion}</span><select disabled={!!running} value={catalogQuestionId} onChange={(event) => onCatalogQuestion(event.target.value)}><option value="">{t.freeQuestion}</option>{catalogQuestions.map((item) => <option key={item.id} value={item.id}>{item.featured ? "★ " : ""}{item.question}</option>)}</select></label>}
    <label className="question-field"><span>{t.question}</span><input disabled={!!running || !!catalogQuestionId} aria-label={t.question} placeholder={t.question} value={question} onChange={(event) => onQuestion(event.target.value)} /></label>
    <div className="action-row run-actions"><button type="button" disabled={!!running || !indexId || (!question && !catalogQuestionId)} onClick={() => onRun("retrieve")}>{running === "retrieve" ? t.retrieving : t.retrieve}</button><button type="button" className="primary-action" disabled={!!running || testingProvider || !providerReady || !indexId || (!question && !catalogQuestionId)} onClick={() => onRun("ask")}>{running === "ask" ? t.generating : t.ask}</button></div>
