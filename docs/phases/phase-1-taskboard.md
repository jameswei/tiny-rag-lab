...
| P1-T12 | M1.6 | Cosine retrieval | P1-T08, P1-T11 | done | claude | known-vector ranking tests pass | reviewed by codex; `uv run pytest tests/test_retrieval.py --tb=short -q`: 19 passed; full suite 172 passed; ranking, zero-vector behavior, and `top_k` validation verified; no findings |
| P1-T13 | M1.6 | `rag index` | P1-T02, P1-T05, P1-T07, P1-T09, P1-T10 | done | claude | CLI builds index artifacts | reviewed by codex; `uv run pytest tests/test_cmd_index_retrieve.py --tb=short -q`: 17 passed; index artifacts, manifest metadata, chunk flags, and help output verified; no findings |
| P1-T14 | M1.6 | `rag retrieve` | P1-T09, P1-T11, P1-T12 | done | claude | CLI prints ranked chunks and scores | reviewed by codex; `uv run pytest tests/test_cmd_index_retrieve.py --tb=short -q`: 17 passed; rank, score, chunk ID, title/path, preview, `top_k`, and help output verified; no findings |
| P1-T15 | M1.7 | Prompt assembly | P1-T12 | done | claude | prompt includes question, context, source markers | reviewed by codex; `uv run pytest tests/test_prompting.py --tb=short -q`: 24 passed; visible template, required instructions, context blocks, source markers, and source table verified; no findings |
