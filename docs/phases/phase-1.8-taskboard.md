...
|---|---|---|---|---|---|---|---|
| P1.8-T01 | M1.8.0 | `failure.py`: `LABEL_*` constants + `RetrieverConfig`, `FailureCase`, `DiagnosisResult`, `DiagnosisReport`, `DetectionThresholds` dataclasses | — | todo | unassigned | All five dataclasses and five constants exist; `dataclasses.asdict()` round-trip for all; all fields JSON-native; `FailureCase` with `gold_doc_ids: []` round-trips correctly; `DiagnosisResult` includes `baseline_retrieved_doc_ids` and `intervention_retrieved_doc_ids` as `list[str]`; default values correct; `uv run pytest tests/test_failure.py -k dataclass --tb=short -q`: N passed | |
