...
| P3.0-T10 | Post-merge repair | Address accepted external-review findings: replayable provider failures, bounded streamed uploads, visible serial job admission, Qdrant error boundaries, diagnostic logging, shared citation extraction, backend-factory use, and legacy manifest coverage. | T01–T09 | done | Codex | Accepted correctness findings have non-secret learner-facing errors and focused regression tests; caching and defensive-only proposals remain explicitly deferred. | Independently signed off by Claude Sonnet 5 on 2026-07-13; `uv run pytest`: 769 passed, 8 skipped (re-run independently); `npm run build`, `docker compose config`, `git diff --check` re-run independently, all clean; findings 1–7 and 10 verified fixed against the diff, 8–9 deferral rationale accepted. |
