...
|---|---|---|---|---|---|---|---|
| P1.5-T01 | M1.5.0 | `rank_bm25` dependency + `BM25Retriever` in `bm25.py` | none | todo | unassigned | `_tokenize` visible and tested; unique-term hit ranks first; empty corpus/query return `[]`; `uv run pytest tests/test_bm25.py --tb=short -q`: N passed | |
| P1.5-T02 | M1.5.1 | `reciprocal_rank_fusion()` + `retrieve_hybrid()` in `hybrid.py` | P1.5-T01 | todo | unassigned | dual-top-1 chunk scores highest after fusion; hybrid returns top_k items; ranks 1-indexed contiguous; `uv run pytest tests/test_hybrid.py --tb=short -q`: N passed | |
| P1.5-T03 | M1.5.2 | `--retriever` flag for `rag retrieve` | P1.5-T01, P1.5-T02 | todo | unassigned | `--help` shows flag; bm25 and hybrid modes return results with fake-backed index; invalid value exits non-zero; `uv run pytest tests/test_cmd_index_retrieve.py --tb=short -q`: N passed | |
| P1.5-T04 | M1.5.2 | `EvalReport.retriever` field + `--retriever` flag for `rag eval` | P1.5-T01, P1.5-T02 | todo | unassigned | `EvalReport` has `retriever` field; `run_retrieval_eval()` accepts retriever param; `rag eval --retriever bm25` and `--retriever hybrid` print valid reports with retriever name; `uv run pytest tests/test_cmd_eval.py tests/test_eval_runner.py --tb=short -q`: N passed | |
