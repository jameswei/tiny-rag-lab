...
120 字符仍是基线默认值，CLI 同时开放了块大小和重叠参数，便于进行实验。
...
- **向量是单位长度。** L2 归一化使余弦相似度可以作为一个简单的点积计算
  —— 查询时不需要除以范数。
- **无需下载、无网络、无 GPU。** 假嵌入器本身只使用 Python 和 NumPy，因此使用
  确定性假后端的测试无需配置模型即可运行。
...
**为什么用 `all-MiniLM-L6-v2`？** 它是一个紧凑的 384 维嵌入模型，可以在 CPU 上
运行，并产生合理的语义向量。对于学习实验室，小且本地胜过大型且依赖 API。
...
将嵌入器放在 ABC 后面的设计选择意味着相同的检索代码可以配合确定性测试中的小型
哈希函数，也可以配合用于语义检索的本地 transformer 模型。这种模式 —— 一个狭窄
的接口加上一个确定性的假实现 —— 贯穿 Phase 1，使流水线无需网络访问即可测试。
